...
def generate_essay_content(details):
    try:
        ...
        print("Invalid yemperature. Using 0.3.")
        temp = 0.3
    ...
    if choice == "1":
        ...
        step_p = generate_response(tep_p, temperature=temp, max_tokens=1024)
        print("\n=== Generated Step-by-Step Body ===\n")
        print(body_step)

        concl_p = f"Write a conclution for an {details['essay_type']} essay about {details['topic']} with the stance of {details['target_audience']}."
        concl = generate_reponse(concl_p, temperature=temp, max_tokens=1024)
